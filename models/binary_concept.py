...
from .helpers import get_mnist, MNIST_CLASSES, MNIST_FEATURES, quantize


# Create model
def binary_concept_net(features: tf.placeholder, weights: Dict[str, tf.Variable]):
    ...
    z_2 = tf.matmul(a_1_b, w_2_b)
    zz = xgemm(a_1_b, w_2_b, otype=tf.float32)
    a_2_b = quantize(z_2)

    # Hidden fully connected layer with 1024 neurons
    w_3_b = quantize(weights['h3'])
    z_3 = tf.matmul(a_2_b, w_3_b)
    a_3_b = quantize(z_3)

    # Output fully connected layer with a neuron for each class
    w_out_b = quantize(weights['out'])
    out_layer = tf.matmul(a_3_b, w_out_b)
    return out_layer


def run_binary_concept_mlp(learning_rate: float,
                           num_steps: int,
                           batch_size: int,
                           display_step: int,
                           hidden_size: int):
